...
Display_Driver_ILI9341 OTM8009A_Driver;
DISPLAY_CONTROLLER_CONFIG OTM8009A_Config;
extern CPU_TO_GRAPHICS_DISPLAY g_Cpu_Display_Driver;


bool Display_Driver_ILI9341::SetWindow(CLR_UINT16 x1, CLR_UINT16 y1, CLR_UINT16 x2, CLR_UINT16 y2)
{
	CLR_UINT16 x1_x2;
...
bool Display_Driver_ILI9341::Initialize()
{
	...
	g_Cpu_Display_Driver.SendCommandToDisplay(ILI9341_Commands::CMD_Power_Control_B, { 0x00, 0x83, 0X30 });
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(3) { 0x00, 0x83, 0X30 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Power_On_Sequence);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(4) { 0x64, 0x03, 0X12, 0X81 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Driver_Timing_Control_A);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(3 ) { 0x85, 0x01, 0x79 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Power_Control_A);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(5) { 0x39, 0x2C, 0x00, 0x34, 0x02 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Pump_Ratio_Control);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x20 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Driver_Timing_Control_B);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(2) { 0x00, 0x00 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Power_Control_1);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x26 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Power_Control_2);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x11 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_VCOM_Control);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(2) { 0x35, 0x3E });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_VCOM_Control_2);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0xBE });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Memory_Access_Control);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x28 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Pixel_Format_Set);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x55 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Frame_Rate_Control_Normal);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(2) { 0x00, 0x1B });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Enable_3G);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x08 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Gamma_Set);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x01 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Positive_Gamma_Correction);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(15) { 0x1F, 0x1A, 0x18, 0x0A, 0x0F, 0x06, 0x45, 0X87, 0x32, 0x0A, 0x07, 0x02, 0x07, 0x05, 0x00 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Negative_Gamma_Correction);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(15) { 0x00, 0x25, 0x27, 0x05, 0x10, 0x09, 0x3A, 0x78, 0x4D, 0x05, 0x18, 0x0D, 0x38, 0x3A, 0x1F });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Column_Address_Set);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(4) { 0x00, 0x00, 0x00, 0xEF });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Page_Address_Set);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(4) { 0x00, 0x00, 0x01, 0x3f });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Memory_Write);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(0) { 0 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Entry_Mode_Set);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(1) { 0x07 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Display_Function_Control);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(4) { 0x0A, 0x82, 0x27, 0x00 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Sleep_Out);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(0) { 0x80 });

	g_Cpu_Display_Driver.SendCommandToDisplay(CMD_Display_ON);
	g_Cpu_Display_Driver.SendDataToDisplay(DisplayData(0) { 0x80 });
...
bool Display_Driver_ILI9341::Uninitialize()
{
	Clear();
	//FIXME: ??
	return TRUE;
}
void Display_Driver_ILI9341::PowerSave(bool On)
{
...
void Display_Driver_ILI9341::Clear()
{
...
void Display_Driver_ILI9341::BitBltEx(int x, int y, int width, int height, CLR_UINT32 data[])
{
...
//void Display_Driver_ILI9341::BitBlt(int width, int height, int widthInWords, CLR_UINT32 data[], bool fUseDelta)
void Display_Driver_ILI9341::BitBlt(int width, int height, int widthInWords, CLR_UINT32 data[])
{
...
void Display_Driver_ILI9341::WriteChar(unsigned char c, int row, int col)
{
...
void Display_Driver_ILI9341::WriteFormattedChar(unsigned char c)
{
	if (c < 32)
	{
		switch (c)
...
CLR_UINT32 Display_Driver_ILI9341::PixelsPerWord()
{
	return ((8 * sizeof(CLR_UINT32)) / g_OTM8009A_Config.BitsPerPixel);
}
CLR_UINT32 Display_Driver_ILI9341::TextRows()
{
	return (g_OTM8009A_Config.Height / Font_Height());
}
CLR_UINT32 Display_Driver_ILI9341::TextColumns()
{
	return (g_OTM8009A_Config.Width / Font_Width());
}
CLR_UINT32 Display_Driver_ILI9341::WidthInWords()
{
	return ((g_OTM8009A_Config.Width + (PixelsPerWord() - 1)) / PixelsPerWord());
}
CLR_UINT32 Display_Driver_ILI9341::SizeInWords()
{
	return (WidthInWords() * g_OTM8009A_Config.Height);
}
CLR_UINT32 Display_Driver_ILI9341::SizeInBytes()
{
