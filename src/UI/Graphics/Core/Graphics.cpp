...
void CLR_GFX_Bitmap::Bitmap_Initialize()
{
	...
	m_palBitmap.clipping.left = 0;
	m_palBitmap.clipping.top = 0;
	m_palBitmap.clipping.right = m_palBitmap.width;
	m_palBitmap.clipping.bottom = m_palBitmap.height;
}
HRESULT CLR_GFX_Bitmap::CreateInstance(CLR_RT_HeapBlock& ref, const CLR_GFX_BitmapDescription& bm)
{
	...
	bitmap = (CLR_GFX_Bitmap*)GraphicsMemoryHeap::Allocate(size);
...
void CLR_GFX_Bitmap::Clear()
{
	...
	memset(m_palBitmap.data, 0, Graphics_Driver::GetSize(m_palBitmap.width, m_palBitmap.height));
}
...
{
	return Graphics_Driver::ConvertNativeToColor(Graphics_Driver::GetPixelNative(m_palBitmap, xPos, yPos));
}
void CLR_GFX_Bitmap::SetPixel(int xPos, int yPos, CLR_UINT32 color)
{
	Graphics_Driver::SetPixelNative(m_palBitmap, xPos, yPos, Graphics_Driver::ConvertColorToNative(color));
}
void CLR_GFX_Bitmap::DrawEllipse(const GFX_Pen& pen, const GFX_Brush& brush, int x, int y, int radiusX, int radiusY)
{
	...
	GFX_Brush nativeBrush = Graphics_Driver::ConvertBrushToNative(brush);
	Graphics_Driver::DrawEllipseNative(m_palBitmap, nativePen, nativeBrush, x, y, radiusX, radiusY);
}
void CLR_GFX_Bitmap::DrawImage(const GFX_Rect& dst, const CLR_GFX_Bitmap& bitmapSrc, const GFX_Rect& src, CLR_UINT16 opacity)
{
	...
	if (dst.Width() == src.Width() && dst.Height() == src.Height())
	{
		...
		if (Graphics_Driver::ClipToVisible(m_palBitmap, xDst, yDst, nWidth, nHeight, &bitmapSrc.m_palBitmap, xSrc, ySrc) == false) return;

		int strideSrc = Graphics_Driver::GetWidthInWords(bitmapSrc.m_palBitmap.width) * 2;
		int strideDst = Graphics_Driver::GetWidthInWords(m_palBitmap.width) * 2;

		CLR_UINT16* ps = (CLR_UINT16*)bitmapSrc.m_palBitmap.data + (strideSrc * ySrc) + xSrc;
		CLR_UINT16* pd = (CLR_UINT16*)m_palBitmap.data + (strideDst * yDst) + xDst;
		...
		if (bitmapSrc.m_palBitmap.transparentColor != PAL_GFX_Bitmap::c_InvalidColor)
		{
			type |= Transparent;
			nativeTransparentColor = Graphics_Driver::ConvertColorToNative(bitmapSrc.m_palBitmap.transparentColor);
		}
			...
			while (nHeight-- > 0)
			{
				...
				for (int col = 0; col < nWidth; col++, ps++, pd++)
				{
					...
					case Translucent_NotTransparent:
						*pd = Graphics_Driver::NativeColorInterpolate(sourceColor, *pd, opacity);
						break;
	...
	else // StretchImage
	{
		...
		if (xOffset < m_palBitmap.clipping.left)
		{
			int deltaX = m_palBitmap.clipping.left - xOffset;
			croppedWidth -= deltaX;
			xNumeratorStart += sourceWidth * deltaX;
			xSourceOffset = xNumeratorStart / dstWidth;
			xNumeratorStart %= dstWidth;
			xOffset = m_palBitmap.clipping.left;
		}

		// Clip right
		//
		if (xOffset + croppedWidth > m_palBitmap.clipping.right)
		{
			croppedWidth = m_palBitmap.clipping.right - xOffset;
		}
		...
		int ySourceOffset = 0;
		int yNumerator = 0;
		if (yOffset < m_palBitmap.clipping.top)
		{
			int deltaY = m_palBitmap.clipping.top - yOffset;
			croppedHeight -= deltaY;
			yNumerator += sourceHeight * deltaY;
			ySourceOffset = yNumerator / dstHeight;
			yNumerator %= dstHeight;
			yOffset = m_palBitmap.clipping.top;
			rowstart += strideSrc * ySourceOffset;
		}

		// Clip bottom
		//
		if (yOffset + croppedHeight > m_palBitmap.clipping.bottom)
		{
			croppedHeight = m_palBitmap.clipping.bottom - yOffset;
		}

		if (croppedWidth <= 0 || croppedHeight <= 0) return;

		CLR_UINT16* pd = (CLR_UINT16*)m_palBitmap.data + (strideDst * yOffset) + xOffset;
		...
		for (int y = 0; y < croppedHeight; y++, pd += strideDst)
		{
			...
			CLR_UINT16 color = *ps;
			CLR_UINT32 nativeTransparentColor = Graphics_Driver::ConvertColorToNative(bitmapSrc.m_palBitmap.transparentColor);
			bool noTransparent = bitmapSrc.m_palBitmap.transparentColor == PAL_GFX_Bitmap::c_InvalidColor;
			...
			for (int x = 0; x < croppedWidth; x++, pd++)
			{
				if (!transparent)
				{
					*pd = (opacity == PAL_GFX_Bitmap::c_OpacityOpaque)
						? color
						: Graphics_Driver::NativeColorInterpolate(color, *pd, opacity);
				}
...
void CLR_GFX_Bitmap::RotateImage(int angle, const GFX_Rect& dstRect, const CLR_GFX_Bitmap& bitmapSrc, const GFX_Rect& srcRect, CLR_UINT16 opacity)
{
	float  cos, sin;
	int     yMin, yMax, xMin, xMax;
	int     index;

	angle %= 360; if (angle < 0) angle = 360 + angle;

	//If it's just a translation, do the BitBlt instead
	if (angle == 0)
	{
		Graphics_Driver::DrawImage(m_palBitmap, dstRect, bitmapSrc.m_palBitmap, srcRect, opacity);
		return;
	}
	...
	for (int y = yMin; y <= yMax; y++)
	{
		...
		for (int x = xMin; x <= xMax; x++)
		{
			...
			if (xSrc <= srcRect.right && srcRect.left <= xSrc && ySrc <= srcRect.bottom && srcRect.top <= ySrc && ySrc > 0 && xSrc > 0)
			{
				...
				CLR_UINT32* pbyteDst = Graphics_Driver::ComputePosition(m_palBitmap, x, y, maskDst, shiftDst);

				CLR_UINT32  mask, shift;
				CLR_UINT32* pbyteSrc = Graphics_Driver::ComputePosition(bitmapSrc.m_palBitmap, xSrc, ySrc, mask, shift);
...
{
	GFX_Pen nativePen = Graphics_Driver::ConvertPenToNative(pen);
	Graphics_Driver::DrawLineNative(m_palBitmap, nativePen, x0, y0, x1, y1);
}
void CLR_GFX_Bitmap::DrawRectangle(const GFX_Pen& pen, const GFX_Brush& brush, const GFX_Rect& rectangle)
{
	GFX_Pen nativePen = Graphics_Driver::ConvertPenToNative(pen);
	GFX_Brush nativeBrush = Graphics_Driver::ConvertBrushToNative(brush);
	Graphics_Driver::DrawRectangleNative(m_palBitmap, nativePen, nativeBrush, rectangle);
}
...
void CLR_GFX_Bitmap::SetPixelsHelper(const GFX_Rect& rect, CLR_UINT32 config, GFX_SetPixelsCallback callback, void* param)
{
	...
	{
		if (Graphics_Driver::ClipToVisible(m_palBitmap, x, y, width, height, NULL, xSrc, ySrc) == FALSE) return;
	}

	int stride = Graphics_Driver::GetWidthInWords(m_palBitmap.width) * 2;
	CLR_UINT16* curRow = ((CLR_UINT16*)m_palBitmap.data) + y * stride + x;
	CLR_UINT16* curPixel;
	...
	for (; height > 0; height--)
	{
		...
		for (x = xStart; x < xEnd; x++)
		{
			...
			if (checkPixelVisible && !Graphics_Driver::IsPixelVisible(m_palBitmap.clipping, x, y))
			{
			...
			else
			{
				...
				{
					*curPixel = Graphics_Driver::NativeColorInterpolate(Graphics_Driver::ConvertColorToNative(color), *curPixel, opacity);
				}
			}
...
void CLR_GFX_Bitmap::Screen_Flush(CLR_GFX_Bitmap& bitmap, CLR_UINT16 x, CLR_UINT16 y, CLR_UINT16 width, CLR_UINT16 height)
{
	...
	CLR_INT32 widthMax = Display::GetWidth();
	CLR_INT32 heightMax = Display::GetHeight();
